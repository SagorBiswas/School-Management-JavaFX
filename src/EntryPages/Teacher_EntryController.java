 ...
 * @author SAGOR
 */
public class Teacher_EntryController implements Initializable {
    ...
    @FXML private ComboBox<String> subcmbx;
    ...
    ObservableList<String>subOpts = FXCollections.observableArrayList();
    
    public void subloader(){
        String sql = "SELECT SUBJECT_NAME FROM SUBJECT";
        try{
            Connection con = new OracleDBMS("SB","sb").getConnection();
            PreparedStatement pst = con.prepareStatement(sql);
            System.out.println("Just Before executng");
            ResultSet rs = pst.executeQuery();
            ResultSetMetaData rsmd = rs.getMetaData();
            System.out.println("Connected in subloader");
            while (rs.next())
            {
                subOpts.add(rs.getString("SUBJECT_NAME"));
            }
            pst.close();
            con.close();
        }
        ...
        {
            //e.printStackTrace();
            System.out.println("Exception in Subloader");
        }
    }
    ...
    void submitButtonAction(ActionEvent event){
        String id,fnme,lnme,bdte,subj,sal,bg,jdte;
        id = T_ID.getText(); fnme = F_NAME.getText(); lnme = L_NAME.getText(); subj = subcmbx.getValue();
        bdte = dobdtp.getValue().toString(); sal = salry.getText(); bg = BGcmbx.getValue(); jdte = jdtp.getValue().toString();
        String sql = "INSERT INTO TEACHERS VALUES(?,?,?,?,TO_DATE(?, 'YYYY-MM-DD'),TO_DATE(?, 'YYYY-MM-DD'),?,?)";
